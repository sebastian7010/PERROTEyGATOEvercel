...
/** Cargar productos desde products-LIMPIO.json **/
async function loadProducts() {
    try {
        const response = await fetch('./products-LIMPIO.json');
        if (!response.ok) throw new Error('Error al cargar products-LIMPIO.json: ' + response.status);
...
function renderCategory(categoryId) {
    ...
    const pageIndex = categoryPages[categoryId] || 0;
...
window.changePage = function(categoryId, direction) {
    ...
    if (typeof categoryPages[categoryId] !== 'number') {
